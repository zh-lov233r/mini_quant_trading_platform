...
创建策略页（`/strategies/new`，含克隆）只显示股票池总数量、前 8 个代码及剩余数量。选择现有股票池仍会把完整代码复制进策略草稿。“调整股票池”打开同一个分页选择器，点击“应用到策略草稿”才接受修改，关闭则丢弃；不会修改原股票池，也不会直接保存策略。空策略股票池仍沿用界面明确提示的“全部符合条件的美国普通股”默认语义。
