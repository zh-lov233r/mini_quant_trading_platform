...
Each completed import synchronizes the active `All A Shares (Tushare)` basket. In `/backtests`, choose any registered strategy and select this basket as the required scope for that run. All nine types use the same native dataset and backtest entrypoint:
