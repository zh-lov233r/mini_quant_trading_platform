...
当前检测器 revision 为 14，状态逻辑 revision 为 4。年度折共 24 个 trial（2 候选 × 3 年 × 4 样本/成本组合），最终最多 6 个 trial。应创建新研究，不继续旧多模式协议或复用其物化。只读漏斗、拒单后续收益审查及可选大盘过滤见[策略规则](support-resistance-strategy.zh-CN.md)。
