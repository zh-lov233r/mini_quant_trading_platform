...
发现阶段仅运行 `support_bounce`。固定 4 组检测器参数（最少 Pivot/跨度、内点容差、斜率上限、区域半宽和时间衰减）与 3 档触发器，共 12 个候选、48 个 trial。确认几何不可修改，收盘破坏或结构冲突重置股票阶段，同一区域可多次执行符合条件的交易。分类不增加可调参数。冠军先要求 2020 年 base/stress 超额收益均为正，再按超额收益、Sharpe、回撤、集中度和参数哈希确定性排序。

默认策略与支撑反弹冠军进入 2021、2022、2023 年度折。校准冠军必须在三个年度折中 base 超额收益全部为正，再按年度超额收益中位数、最差回撤、stress 衰减和哈希冻结。最终子实验仅生成样本外 trial，并加入与 base 成本相同的 `base_cache_replay`，逐项比较事件、信号、交易、持仓和 NAV。实际排队最多 78 个 trial，空余预算不重新分配。
...
当前检测器 revision 为 13，状态逻辑 revision 为 4。年度折共 24 个 trial（2 候选 × 3 年 × 4 样本/成本组合），最终最多 6 个 trial。应创建新研究，不继续旧多模式协议或复用其物化。只读漏斗、拒单后续收益审查及可选大盘过滤见[策略规则](support-resistance-strategy.zh-CN.md)。
