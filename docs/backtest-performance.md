...
The native `run_backtest(dataset, strategy, options, control_callback)` foundation now covers eight strategies: Trend, Mean Reversion, Momentum Breakout, Island Reversal, Double Bottom, Head-and-Shoulders Bottom, Rounded Bottom, and V Reversal. It reads the two PreparedDataset v3 NumPy buffers without copying, releases the GIL for the trading loop, and reacquires it only once per completed formal session for cancellation/progress. Rows before `start_date` warm the bounded pattern histories, incremental Double Bottom candidates/best-pattern state, and dynamic-universe history counts without producing signals, trades, equity rows, or callbacks. Its typed `KernelResult` exposes read-only NumPy views for signals, trades, equity, positions, and per-session dynamic-universe diagnostics. The shared native ledger preserves T-close/T+1-open timing, SELL-first execution, shared cash, deterministic strength ranking and thresholds, position limits, cumulative three-stage targets, same-setup/no-regression entry rules, minimum commissions, adverse slippage, split quantity/cost-basis adjustment, stable instrument identity, missing opens, and delisting write-offs. Point-in-time membership uses the same deterministic exclusion order and per-instrument processed-session history counter as Python, filters BUY signals only, and keeps ineligible existing positions exit-only. Pattern trades add setup/stage columns; position and BUY audit output retains the original entry date plus cumulative canonical entry history, pre/post quantities, and weighted average cost. Frozen Python golden records and full Python/C++ differentials cover all five staged strategies, including a 20-symbol by 120-session matrix with `1e-10` numeric tolerance.

The 2026-09-01 no-database synthetic smoke on Darwin arm64 with Python 3.12.7 ran that five-strategy 20-by-120 differential in 1.49 seconds wall time (the unittest body reported 1.162 seconds). This is a comparison marker only, not the final performance gate. The production worker still uses the Python backtest path and existing persistence; typed COPY, the full Support/Resistance ledger, and the one-time cutover remain later work.

Support/Resistance uses the native module as the single implementation for detector cache identity, full-Pivot zone identity, `NUMERIC(24,10)` half-up price normalization, deterministic weighted Theil-Sen fitting, frozen-zone projection validation, and cross-session candidate, regime, entry-channel, exit, and audit evolution. The public native catalog and daily evaluator therefore cover all nine engine-ready strategies. Existing Python dataclasses remain the Support/Resistance transport and persistence shape while the native state machine mutates them in place; its complete typed `run_backtest` ledger is still pending. There is no runtime engine selector or Python algorithm fallback for migrated behavior.
