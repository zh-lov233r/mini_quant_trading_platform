...
Signals, transactions, snapshots, Support/Resistance zone/regime versions, shared lifecycle events, and run decision events use the current SQLAlchemy Session's `postgresql+psycopg` connection for `COPY FROM STDIN` in batches of 5,000. Native audit output supplies typed query columns and one canonical JSON string per event; Python performs one strict boundary validation and passes that original JSON to COPY. Batches never commit independently: validation, COPY, cancellation, materialization, linking, or summary failure rolls back the run's complete result. Audit schema v2 stores `touch`, `invalidation`, `phase_ended`, `regime_transition`, `entry_channel_started`, and `entry_channel_ended` once per materialization. Cache hits skip zone, regime, and shared-event traversal/validation and write only run-scoped decision events. Old materializations remain readable for historical runs but cannot satisfy a v2 cache lookup.
...
Apply the additive schemas before routing traffic:
...
The project has no Alembic workflow. Before apply, resolve and record the exact target database, take a database-native backup, review current constraints/indexes, and run the SQL with `ON_ERROR_STOP`. `create_zzzzzz_support_resistance.sql` adds audit format versioning plus `support_resistance_materialization_events`; existing materializations become version 1 and are not rewritten, while new calculations use version 2. New installations get `market_data_maintenance_state` from `create_zzzzzzz_backtest_performance.sql`. Existing installations use the reviewed, incremental `backend/utils/remove_market_data_fingerprints.sql`; it retires fingerprint columns and `data_changed`, invalidates every existing Support/Resistance materialization without deleting linked history, and creates the current-row partial unique index. SQL is never applied automatically. The scripts deliberately do not add speculative signal/transaction cursor indexes or rewrite historical run detail.
...
| `first_chunk_ready_ms` / `chunk_load_ms` / `chunk_compute_ms` | Time to the first usable year and per-year load/compute walls. |
| `producer_wait_ms` / `consumer_wait_ms` / `pipeline_overlap_ms` | Depth-one queue backpressure, compute-side starvation, and measured intersection of load and compute intervals. |
| `signal_persist_ms` / `transaction_persist_ms` / `snapshot_persist_ms` | Independent writes for the existing typed detail tables. |
| `support_resistance_materialization_events` / `support_resistance_run_events` | Shared rows created for a new materialization and dynamic rows written for this run. |
...
Manual, research and verification backtests share one COPY reader. It produces v5 Fortran-order int64/float64 chunks accepted by native ABI 3. Manifests use `read_path_revision=binary-shards-3`; previous caches do not match new requests and are not deleted automatically. A complete forward-adjusted OHLC series already incorporates split-style actions, so the ledger does not adjust its quantity again. A fully unadjusted series keeps the native quantity and average-cost adjustment. Mixed adjusted/unadjusted OHLC before an action fails preparation because neither ledger treatment would preserve one price basis.

The producer emits one clipped calendar-year chunk at a time for the complete resolved instrument set into a queue of depth one. The consumer starts native computation after the first non-empty chunk while the producer prepares the next. Cold chunks import one exported PostgreSQL snapshot and retain the existing maintenance read lock. Each completed chunk is published atomically and can be reused; failed or cancelled unpublished temporary directories are removed. Empty years do not enter the native session. Once a chunk is consumed, its large arrays are unmapped; Support/Resistance retains only compact instrument, symbol, and session-date columns needed for persistence validation.
...
Native audit output provides typed event/date/zone/score/price columns, a materialization-event flag, and one read-only canonical JSON string per row. Persistence validates each selected row once and streams its original JSON through the existing 5,000-row COPY batches. On reuse, the native flag selects only dynamic rows without iterating or decoding shared lifecycle payloads. Geometry, identity, strict-JSON validation and transaction rollback remain in place.

The existing progress schema reports native export at 85–88%, detail preparation at 88%, zone/regime/event persistence at 90–98%, and commit at 99%. Export counts instruments; persistence counts rows within its stage group. Cancellation is checked before write batches. Audit schema v2 is an additive DDL change and does not rewrite historical events; apply it through the documented backup/preflight procedure. Rebuild the local wheel with `.venv/bin/pip install --no-build-isolation --no-deps -e backend/native` and restart the backtest manager after active tasks are stopped. Cancelled runs remain cancelled; rebuilding does not retry them. Keep both paper-scheduler flags disabled when starting the backend for verification.
