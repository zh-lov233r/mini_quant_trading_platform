...
class MarketDataQualityHelpersTest(unittest.TestCase):
    ...
    def test_exit_policy_blocks_failures_but_not_default_warnings(self):
        self.assertFalse(should_fail({"status": "PASS"}, strict=False))
        self.assertFalse(should_fail({"status": "WARN"}, strict=False))
        self.assertTrue(should_fail({"status": "WARN"}, strict=True))
        self.assertTrue(should_fail({"status": "FAIL"}, strict=False))
