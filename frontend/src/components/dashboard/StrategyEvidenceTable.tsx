...
export function StrategyEvidenceTable({ rows }: { rows: DashboardStrategyEvidence[] }) {
  ...
  const columns: DenseDataColumn<DashboardStrategyEvidence>[] = [
    { id: "strategy", header: t("dashboard.strategy"), accessor: r => r.name, width: 200, cell: (_, r) => <div className={styles.cell}><Link href={`/strategies/${r.strategy_id}`}>{r.name}</Link><span>v{r.version}</span></div> },
