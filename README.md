...
- Paper trading
  ...
  - Convert Paper history into an in-memory PreparedDataset v5 and call native `evaluate_day(dataset_day, strategy, portfolio_state)`, sharing the same rules and canonical metadata as backtests; broker queries, idempotent orders, sleeve isolation, and next-valid-session-open quote validation remain in Python
