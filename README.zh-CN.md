...
- Agent 辅助策略研究
  ...
  - 股票阶段身份与四状态分类独立；收盘破坏或区域冲突结束整个旧阶段，独立有效性研究采用检测器 revision 13
...
回测详情把个股盈亏、信号强度排名、生命周期、交易明细和最新持仓收纳进同一个复盘工作台，通过标签页切换，并默认首先显示“生命周期”。窗口内容区独立滚动，并保留各模块原有的筛选、排序、分页和展开交互；只有点击生命周期摘要行（或聚焦该行后按 Enter / 空格）才会展开或收起；详情文字、图表、空白区及拖拽都不会触发折叠。切换买入前或卖出后显示范围时会保留现有蜡烛图和窗口滚动位置，待新数据返回后原位更新。生命周期图把所有颜色的事件圆点和买卖箭头放在价格区上下留白，并用虚线连接对应 K 线，避免遮挡蜡烛和成交量；相邻标记文字会自动换边或错层，避免相互覆盖。
...
工作台动效使用现有 CSS Modules 和 Radix：操作反馈、浮层开合与内容进入分别采用 120ms、180ms 和 240ms。可点击的策略与回测卡片在鼠标悬停或键盘聚焦时轻微上浮，并沿用策略大类颜色高亮；总览四张指标卡以 30ms 间隔淡入，骨架屏仅在加载时轻微明暗变化。轮询、搜索和分页不会重播整页动画，金融数值直接显示，图表与虚拟表格行不做进场变形。进度条只衔接服务端实际百分比，终态立即停稳；居中操作通知仍在 3200ms 后自动移除。弹窗退出动画结束后恢复焦点与页面滚动，手机保留全屏布局。系统开启“减少动态效果”时停用动效与平滑返回顶部，操作和状态信息仍完整可用。本次仅调整前端呈现，无 API、数据库或券商操作变化。

股票库支持编辑已保存组合的名称、说明、股票代码和状态（`PUT /api/stock-baskets/{basket_id}`），保留组合 ID；历史回测快照不变。编辑按钮位于每张组合卡片左下角。创建与编辑均使用短 ticker / 公司名搜索、单个代码添加、市场 / 行业 / 市值筛选和每页 20 只的已选列表，不再存在完整 ticker 长文本框。全部匹配需确认后追加并去重；关闭而不保存会丢弃草稿，说明输入不会重渲染已记忆化的选择器。系统自动维护的 `All Common Stock` 不可编辑，请另建独立组合。数据单位、手动更新和授权建表见[股票池筛选](docs/stock-basket-screening.zh-CN.md)。创建和克隆策略只配置交易逻辑，不再选择股票范围；每次新建手工回测必须选择已保存股票池，并把当次成员固化到运行快照。回测工作台支持 A 股 / 美股的结果筛选和组合选择筛选；结果市场来自运行快照，与当前组合名称无关。任务中心的“总耗时”使用开始执行到完成持久化的时间，不包含排队；缺失时间戳显示“—”或“-”，不估算。行情入口位于左侧栏底部，收起导航时显示图标，手机上位于左下角；行情窗默认从工作区左侧打开。

百分比参数以百分数展示和输入（输入 10 表示 10%，0.5 表示 0.5%），保存时仍为比例小数；收益指标保留必要的小数精度，ATR、成交量倍数、bps 和原始 JSON 不换算。总览的策略证据与回测活动、研究创建的大类选项复用策略库颜色。SR Pivot 标记仅在悬停或固定区域时显示，表示事后拟合依据，不表示极值当日已知的交易信号。
